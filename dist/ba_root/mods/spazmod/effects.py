...
class SurroundBall(ba.Actor):
    def __init__(self, spaz, shape="bones"):
        ...
        self.node = ba.newnode("prop", attrs={"model": s_model, "body": "sphere", "color_texture": s_texture,
                                              "reflection": "soft", "model_scale": 0.5, "body_scale": 0.1,
                                              "density": 0.1, "reflection_scale": [0.15], "shadow_size": 0.6,
                                              "position": spaz.node.position, "velocity": (0, 0, 0),
    ...
    def getTargetPosition(self, spazPos):
        p = spazPos
        pt = (p[0] + self.surroundRadius * math.cos(self.curAngle), p[1] + self.curHeight,
        ...
        if (self.curHeight > self.heightMax) or (
                self.curHeight < self.heightMin): self.curHeightDir = -self.curHeightDir
        return pt
...
class premiumsBall(ba.Actor):
    def __init__(self, spaz, shape="bones"):
        ...
        self.node = ba.newnode("prop",
                               attrs={"model": ba.getmodel("shield"),
                                      ...
                                      "velocity": (0, 0, 0),
                                      "materials": [SharedObjects.get().object_material, factory.pballMaterial]},
                               delegate=self)
        ...
        ba.animate_array(node=self.shield, attr='color', size=3,
                         keys={0: (
                         random.choice([1, 2, 3, 4, 5, 6, 7, 8, 9]), random.choice([1, 2, 3, 4, 5, 6, 7, 8, 9]),
    ...
    def getTargetPosition(self, spazPos):
        p = spazPos
        pt = (p[0] + self.premiumsRadius * math.cos(self.curAngle), p[1] + self.curHeight,
              p[2] + self.premiumsRadius * math.sin(self.curAngle))
        self.curAngle += self.angleDelta
        self.curHeight += self.heightDelta * self.curHeightDir
        if (self.curHeight > self.heightMax) or (
                self.curHeight < self.heightMin): self.curHeightDir = -self.curHeightDir
...
class Effect(ba.Actor):
            ...
            if cl_str in pats and _settings['enableTop5effects']:
                ...
                if rank < 6:
                    if rank == 1:
                        ...
                        self.surround = SurroundBall(spaz,
                                                     shape="bones")  # self.neroLightTimer = ba.Timer(500, ba.WeakCall(self.neonLightSwitch,("shine" in self.Decorations),("extra_Highlight" in self.Decorations),("extra_NameColor" in self.Decorations)),repeat = True, timetype=tt, timeformat=tf)
                    elif rank == 2:
                        ...
                        self.addLightColor((1, 0.6, 0.4));
                        self.checkDeadTimer = ba.Timer(150, self.checkPlayerifDead, repeat=True, timetype=tt,
                                                       timeformat=tf)
    ...
    def update_Scorch(self):
        ...
        if spaz is not None and spaz.is_alive() and spaz.node.exists():
            ...
            if not hasattr(self, "scorchNode") or self.scorchNode == None:
                ...
                self.scorchNode = ba.newnode("scorch",
                                             attrs={"position": (spaz.node.position), "size": 1.17, "big": True})
                spaz.node.connectattr("position", self.scorchNode, "position")
            ba.animate_array(self.scorchNode, "color", 3, {0: self.scorchNode.color, 500: color}, timetype=tt,
                             timeformat=tf)
        else:
    ...
    def neonLightSwitch(self, shine, Highlight, NameColor):
        ...
        if spaz is not None and spaz.is_alive() and spaz.node.exists():
            color = (random.random(), random.random(), random.random())
            if NameColor: ba.animate_array(spaz.node, "nameColor", 3,
                                           {0: spaz.node.nameColor, 500: ba.safecolor(color)}, timetype=tt,
                                           timeformat=tf)
            ...
            if Highlight:
                ...
                color = (random.random(), random.random(), random.random())
                if shine: color = tuple([min(10., 10 * x) for x in color])
                ba.animate_array(spaz.node, "highlight", 3, {0: spaz.node.highlight, 500: color}, timetype=tt,
                                 timeformat=tf)
        else:
    ...
    def emitDistortion(self):
        ...
        ba.emitfx(position=spaz.node.position, velocity=spaz.node.velocity, count=random.randint(1, 5),
    ...
    def rainboweffect(self):
        ...
        ba.animate_array(spaz.node, 'color', 3,
                         {0: (random.choice([1, 2, 3, 4, 5, 6, 7, 8, 9]), random.choice([1, 2, 3, 4, 5, 6, 7, 8, 9]),
                              random.choice([1, 2, 3, 4, 5, 6, 7, 8, 9])),
                          0.2: (2, 0, 2),
                          0.4: (2, 2, 0),
                          0.6: (0, 2, 0),
    ...
    def emitSpark(self):
        ...
        if spaz is None or not spaz.node.exists():
            self.handlemessage(ba.DieMessage())
            return
        ba.emitfx(position=spaz.node.position, velocity=spaz.node.velocity, count=random.randint(1, 10), scale=2,
                  spread=0.2, chunk_type="spark")
    ...
    def emitIce(self):
        ...
        if spaz is None or not spaz.is_alive() or not spaz.node.exists():
            self.handlemessage(ba.DieMessage())
            return
        ba.emitfx(position=spaz.node.position, velocity=spaz.node.velocity, count=random.randint(2, 8), scale=0.4,
    ...
    def emitSlime(self):
        ...
        if spaz is None or not spaz.is_alive() or not spaz.node.exists():
            self.handlemessage(ba.DieMessage())
            return
        ba.emitfx(position=spaz.node.position, velocity=spaz.node.velocity, count=random.randint(1, 10), scale=0.4,
                  spread=0.2, chunk_type="slime")
    ...
    def handlemessage(self, m):
        ...
        if isinstance(m, ba.OutOfBoundsMessage):
            ...
            if not self._hasDead:
                ...
                try:
                    if killer in (None, ba.Player(
                        None)) or killer.actor is None or not killer.actor.exists() or killer.actor.hitPoints <= 0: killer = None
                except:
                    killer = None
